      ...
      <v-dialog v-model="dialog" persistent max-width="600px" >
        <v-card color="#dfdce6" class="mx-lg-auto rounded-lg">
          ...
          <v-card-actions class="pa-7 d-flex justify-center">
            <v-spacer></v-spacer>
            <v-spacer></v-spacer>
            <v-btn x-large color="#4c4c65" class="white--text" @click="dialog = false; addSchedule(scheduleInfo);">확인</v-btn>
            <v-spacer></v-spacer>
            <v-btn x-large color="#FFFFFF" class="black--text" @click="dialog = false; addSchedule(null);">취소</v-btn>
            <v-spacer></v-spacer>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  created() {
    this.scheduleInfo.start_date = new Date().toISOString().substr(0, 10),
    this.scheduleInfo.end_date = new Date().toISOString().substr(0, 10)
  }
}
</script>
